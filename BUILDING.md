...
**The build says which one it picked**, in the step name:
...
That is not decoration. Until T0125 the interop probe misread procfs and
silently lost to wine on every WSL machine, and the only trace was a wine
startup warning that looked like it belonged to something else. A fallback you
cannot see is one you cannot notice going wrong, so the choice is now stated
rather than inferred.

## The API reference

`docs/api/` is a generated markdown reference to every public symbol in
`engine/include/hp/`, written for a coding agent rather than a human browser.
**It is regenerated by the ordinary build** — `zig build`, `zig build linux`,
`zig build windows`, `zig build all` — so there is no command to remember and it
cannot silently fall behind a header.

```sh
zig build docs                  # regenerate + check, on its own
zig build docs -Ddocs-baseline  # deliberately rewrite the defect baseline
```

It is cached on its real inputs: the public headers, the generator, the
baseline, and the generated markdown itself. Changing a `.cpp`, a test or a
document does not re-run it. Changing a public header does — and so does
deleting or hand-editing `docs/api`, which is why the output is an input.
Expect a regeneration to cost one extra run before it settles back to `cached`;
the run that rewrites the output necessarily invalidates its own inputs.

**The check runs by default and fails the build.** Every public declaration must
carry a doc comment, and an `@param` must name a parameter that exists — a
`@param` naming a renamed parameter is documentation that is confidently wrong,
which misleads an agent worse than none. Existing gaps are held in
`tools/api_docs_baseline.txt`, a ratchet that may shrink and never grow, so new
public API must be documented from its first commit. Do not add a new defect to
...
so a stale reference cannot reach main either way.

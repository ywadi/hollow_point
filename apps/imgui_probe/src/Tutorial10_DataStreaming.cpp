...
// HP_DOCKING_PROBE -- the only edit to the copied tutorial.
//
// Proves the swapped-in docking build of ImGui is live: ImGuiConfigFlags_
// DockingEnable and DockSpaceOverViewport() exist only on the docking branch,
// so this fails to compile against the 1.92.1 DiligentEngine bundles.
//
// HP_PROBE_EXIT_FRAMES makes the app quit on its own after N frames, so it can
// be verified on a headless display without needing to be killed.
namespace
{

void HP_DockingProbe()
{
    ImGuiIO& io = ImGui::GetIO();
    io.ConfigFlags |= ImGuiConfigFlags_DockingEnable;

    // Central node passthrough keeps the 3D scene visible behind the dockspace.
    ImGui::DockSpaceOverViewport(0, ImGui::GetMainViewport(), ImGuiDockNodeFlags_PassthruCentralNode);

    // A second window exists purely so there is something to dock.
    if (ImGui::Begin("Docking Probe"))
    {
        ImGui::Text("ImGui %s", ImGui::GetVersion());
        ImGui::Text("docking enabled: %s",
                    (io.ConfigFlags & ImGuiConfigFlags_DockingEnable) ? "yes" : "no");
        ImGui::Text("drag this window's title bar onto another to dock it");
    }
    ImGui::End();

    if (const char* Frames = std::getenv("HP_PROBE_EXIT_FRAMES"))
    {
        static int FrameCount = 0;
        if (++FrameCount >= std::atoi(Frames))
        {
            std::printf("HP_PROBE: %d frames rendered, ImGui %s, docking %s -- exiting\n",
                        FrameCount, ImGui::GetVersion(),
                        (io.ConfigFlags & ImGuiConfigFlags_DockingEnable) ? "ON" : "OFF");
            std::fflush(stdout);
            std::exit(0);
        }
    }

...
bool buildModuleSignatureDesc(const ModuleSignatureRequest& request,
    ...
    textures.clear();
    if (request.stages == nullptr || request.stages->empty() ||
        request.engineResources == nullptr) {
        return false;
    }
    for (const ModuleSignatureStage& stage : *request.stages) {
        if (stage.shader == nullptr) {
            return false;
        }
    }

    // **Collected before anything is added, because a resource may be named by
    // more than one stage** (T0146). One `AddResource` per name with the union
    // of its stage bits: adding the same name twice is a duplicate-resource
    // error at signature creation, and adding it once with only the first
    // stage's bit makes the *other* stage's use fail PSO creation instead.
    struct Collected {
        std::string name;
    ...
    for (const ModuleSignatureStage& stage : *request.stages) {
        const Diligent::Uint32 count = stage.shader->GetResourceCount();
        for (Diligent::Uint32 i = 0; i < count; ++i) {
            Diligent::ShaderResourceDesc resource;
            stage.shader->GetResourceDesc(i, resource);
            ...
            // collects it or refuses it **by name** — the silent third option
            // is the one D35 exists to remove.
            ...
            const auto found =
                std::find_if(collected.begin(), collected.end(), [&](const Collected& entry) {
                    return entry.name == resource.Name;
                });
            if (found != collected.end()) {
                if (found->type != resource.Type || found->arraySize != resource.ArraySize) {
                    // One name, two shapes across the two stages: impossible
                    // from one declaration, so this means the reflection is
                    // describing two different things and the signature would
                    // be a guess. Named rather than resolved by picking one.
                    HP_LOG_ERROR(kLog,
                                 "module '{}' declares '{}' as different resource kinds in "
                                 "different stages; one declaration must have one shape",
                                 request.moduleName, resource.Name);
                    return false;
                }
                found->stages |= stage.stage;
                continue;
            }
            collected.push_back(Collected{std::string{resource.Name}, resource.Type,
                                          resource.ArraySize, stage.stage});
        }
    }
    ...
    for (const Collected& entry : collected) {
        built.AddResource(entry.stages, entry.name.c_str(), entry.arraySize, entry.type,
                          Diligent::SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE);
        if (entry.type == Diligent::SHADER_RESOURCE_TYPE_TEXTURE_SRV) {
            builtTextures.push_back(entry.name);
        }
    }

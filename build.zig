...
//!   zig build docs            regenerate + check docs/api/ (the C++ API) and
//!                             docs/shaders/ (the shader contract), both agent-facing
...
pub fn build(b: *Build) void {
    ...
    // --- shader contract docs (T0141.14) ------------------------------------
    //
    // The same mechanism, for the surface a game's *shader* is written against.
    // Separate from the C++ reference for one concrete reason and one design
    // one: it needs **python3 alone**, no libclang, so it stays honest on a
    // host where the C++ generator is skipped; and the boundary it enforces is
    // different -- there is no `include/` vs `src/` split in `engine/shaders/`,
    // so `gen_shader_docs.py` reads a marker comment in each file and refuses
    // to run if one is missing. That refusal is the point: a reference that
    // documented `HpSurface.slang`'s internals would make the engine's
    // implementation a public contract, which is exactly what D27 exists to
    // prevent.
    const shader_docs = b.addSystemCommand(&.{
        "python3",
        "tools/gen_shader_docs.py",
        "--shaders",
        "engine/shaders",
        "--out",
        "docs/shaders",
    });
    shader_docs.setName("generate shader contract docs");
    _ = shader_docs.addPrefixedOutputFileArg("--stamp=", "shader-docs.stamp");
    shader_docs.addFileInput(b.path("tools/gen_shader_docs.py"));
    addDirInputs(b, shader_docs, "engine/shaders", ".slang");
    // Generated output declared as an input, for the same reason the API
    // reference does it: zig's declared output is a stamp in the cache, so
    // without this `rm -rf docs/shaders && zig build docs` scores a cache hit
    // and restores nothing. Measured on the C++ side (T0123), inherited here.
    addDirInputs(b, shader_docs, "docs/shaders", ".md");

    const docs_step = b.step("docs", "Generate the markdown API reference for coding agents");
    docs_step.dependOn(&docs.step);
    docs_step.dependOn(&shader_docs.step);

    // Wired into every route to a build independently of libclang, because
    // this generator does not need it -- a host without libclang still cannot
    // leave the shader contract stale.
    if (b.findProgram(&.{"python3"}, &.{})) |_| {
        b.getInstallStep().dependOn(&shader_docs.step);
        all_step.dependOn(&shader_docs.step);
        for (target_steps.items) |ts| ts.dependOn(&shader_docs.step);
    } else |_| {
        std.debug.print(
            "warning: python3 not found -- docs/shaders will not be regenerated by this " ++
                "build.\n  CI's api-docs-current job remains the backstop.\n",
            .{},
        );
    }
...
/// Every file with `ext` in `sub`, named to the build graph as an input.
///
/// The generalisation of `addPublicHeaderInputs`/`addGeneratedDocInputs` for
/// the shader-contract generator (T0141.14), and it keeps both of their
/// lessons: enumerate rather than hardcode, so a file added tomorrow is
/// covered; and when the directory is missing or empty, force a run rather
/// than declaring nothing -- declaring nothing is the subtle wrong answer,
/// because the input set then collapses back to what the first run hashed and
/// a deleted output scores a cache hit.
fn addDirInputs(b: *Build, run: *Step.Run, sub: []const u8, ext: []const u8) void {
    var dir = b.build_root.handle.openDir(b.graph.io, sub, .{ .iterate = true }) catch {
        run.has_side_effects = true;
        return;
    };

...
| normal map convention (green channel, mirrored shells) | ✅ | **closed 2026-08-08 (T0168.5)**: green = +Y = up the image, glTF's own OpenGL convention, and the bitangent follows the chart on a mirrored UV shell — both through `HpTangentFrameGrad`, both pinned by `tangent_frame_test.cpp`'s lit case. **A deliberate divergence from vendored DiligentFX**: upstream's `TransformTangentSpaceNormalGrad` builds `b = cross(t, n)`, a fixed chirality that points down-image on a plain chart, so upstream applies glTF maps green-inverted and mirror-blind; the engine's surface stage no longer calls it. The artist-facing answer is in `11-material-format.md` | — |
...
| `KHR_draco_mesh_compression` | ✅ full decode behind `TINYGLTF_ENABLE_DRACO` | ✅ target supplied | ✅ **closed 2026-08-08 (T0168.3)**: draco 1.5.7 vendored as a submodule at exactly the ref Diligent's FetchContent declares, redirected via `FETCHCONTENT_SOURCE_DIR_DRACO` (the EnTT/abseil pattern — offline configure holds, asserted by CI's `-src` check), `DILIGENT_ENABLE_DRACO` on. Proven end to end in `import_coverage_test.cpp`: a draco-required file synthesised with the same library's encoder decodes (82 bytes → 4 points / 2 faces), renders full-frame, and raises no required-extension warning — on both targets, so the decode also runs inside the cross-compiled DLL | — |

...
|---|---|---|---|
| 1 | [T0170](open/0170-diligent-owns-the-render-loop.md) | Diligent owns the render loop | **Six capabilities in three days were already in the tree with the switch off** — spec-gloss, `TANGENT.w`, the tangent frame, Draco, the green channel, and **order-independent transparency**, which sat behind `OITLayerCount = 0` while this project prepared to hand-write a strictly worse sort. The cause is structural: **D26** let a constraint on the *shader* swallow the *submission loop*, and every lost capability was invisible from inside our own walk. Measured: `RenderInfo::AlphaModes` gives D37's snapshot and `CreateCustomSignature` gives T0161's signatures — **the loop was never blocked, only the material shader is**. Subclass `GLTF_PBR_Renderer` and OIT, bucketing, the PSO cache and IBL all arrive at once |
| 2 | [T0045](open/0045-culling-and-render-queues.md) | culling and render queues | **No longer the movable one — a real asset made the missing sort visible.** T0167's Aston Martin renders its own interior through its own glass, and which pieces survive changes with the camera ([frame](findings/t0045-blend-order-aston.png), and the ticket's 2026-08-08 note). The blend pass draws in submission order because T0147 split 10.9 and deliberately refused to invent half a sort. **The frame also constrains 45.4**: the glass and the interior are primitives of the *same* draw item under one material, so a per-object sort key still gets it wrong — the sort has to reach primitives. It still needs nothing from the surface or lighting stages, and **T0152.5 adds one constraint**: facing is per-draw (the determinant rule), so any batching or sorting must keep the per-draw cull decision |
| 3 | [T0087](open/0087-environment-lighting.md) | environment lighting, IBL and skybox | **Moved up on the owner's call, 2026-08-08, on measured evidence.** T0167's car is dark because its paint colour is authored into the *specular* map (`texture_1` bright cyan, diffuse navy-black) and Sketchfab's teal is that map reflecting **their** environment — the workflow misread was real and T0168.2 already fixed it, so what is left is purely the missing environment. Not asset-specific: a metal with nothing to reflect is black by definition, and **every** PBR export assumes an environment. It also makes a shipped family visible again — T0143 qualifies clearcoat-as-lacquer and sheen-as-velvet as *punctual-only*, and the volume family is authored, packed and debug-visible while shading nothing, with a test pinning "changes no shaded pixel" that this ticket flips. **Ahead of T0086 deliberately**: shadow bias tuned with no ambient term is tuned against pure black, and re-baselining against a wrong baseline is the trap T0152 recorded. Cheaper than Moderate suggests — `HpResolveLighting` is a named seam that `#error`s until the term is written, and the Charlie-LUT embed has a worked pattern from T0143 |
...
| 459 | [T0170](open/0170-diligent-owns-the-render-loop.md) | DiligentEngine owns the render loop; the engine owns the shader | 4 — Render layer | 🔜 TODO | High | Complex |

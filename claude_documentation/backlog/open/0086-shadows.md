...
| **Order** | 480 |
| **Created** | 2026-08-03 |
| **Inherits a decision from** | [../completed/0096-hdr-pipeline-and-tonemapping.md](../completed/0096-hdr-pipeline-and-tonemapping.md) — **86.7's four filter modes are selected by a compile-time macro (`SHADOW_MODE`), not a constant-buffer field**, exactly like `TONE_MAPPING_MODE`. T0096 is deciding how this engine handles that class of switch (one PSO per value, a project-wide compile-time choice, or a runtime branch); take its answer rather than inventing a second. T0155's `TEXTURING_MODE` is the third instance |

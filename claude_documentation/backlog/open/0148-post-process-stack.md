...
| **Blocked by** | [0094-gameplay-extensible-rendering.md](0094-gameplay-extensible-rendering.md) — **for the game-facing half only** (148.9). The engine chain can be built first; what must not happen is this ticket inventing a second pass-insertion mechanism, which is exactly what **D35**'s first half forbids |
